<template>
  <b-modal :id="'addCH'+propMaCH" hide-footer :title-html="title">
    ...
    <div class="input-group mb-2">
      ...
      <select class="form-control" v-model="maloaich">
...
@Component
export default class ThemCHC extends Vue {
  @Prop()
  private propMaCH: any;
  @Prop()
  private propCHCLength: any;
  private tenchc = "";
  private diem = 0;
  private maloaich = "";
  private hinhanh = "";
  private linkaudio = "";
  private title =
    "<b style='color: #ff69b4; text-shadow: 1px 1px 1px black;'>Thêm câu hỏi con</b>";
  ...
  get getMaCHC() {
    return (
      this.propMaCH +
      (this.propCHCLength + 1 < 10
        ? "0" + (this.propCHCLength + 1)
        : this.propCHCLength + 1)
    );
  }
  ...
  addChcApi() {
    if (this.getTenCHC!= "" && this.getMaLoaiCH!= "") {
      if (this.uploadImg == 100 && this.uploadAudio == 100) {
        axios
          .get("https://backend-fois-smile.herokuapp.comchildquestion/add", {
            params: {
              machc: this.getMaCHC,
              tenchc: this.getTenCHC,
              diem: this.getDiem,
              mach: this.propMaCH,
              maloaich: this.getMaLoaiCH,
          ...
          .then((res) => {
            if (res.data == true) {
              alert("Đã thêm câu hỏi con thành công!!!");
              this.$emit("reloadCHC");
              this.$bvModal.hide('addCH'+this.propMaCH);
            }
          });
      }
      else{
