...
export default class Comment extends Vue {
  ...
  get getCommentAPI() {
    this.replyFlag = -1;
    axios
      .get("http://localhost:3000/comment/getcomment", {
        params: {
          mabh: this.propMaBH,
        },
      })
  ...
  async addComment() {
    await axios
      .get("http://localhost:3000/comment/addcomment", {
        params: {
  ...
  connect() {
    this.stompClient = Stomp.over(new SockJS("http://localhost:3000/websocket"));
